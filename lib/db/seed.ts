  ...
  try {
    ...
    const categoryData: NewCategory[] = [
      ...
      },
      {
        ...
        slug: "business",
        description:
          "Business strategies, entrepreneurship, and market insights",
      },
    ];
...
Start building amazing applications with Next.js 15 today!`,
        ...
        categoryId: insertedCategories[1].id, // Web Development
        status: "published",
...
Stay ahead of these trends to create websites that users love!`,
        ...
        categoryId: insertedCategories[2].id, // Design
        status: "published",
...
These practices will help you build robust, type-safe applications that scale.`,
    ...
    // Seed Blog Post Tags (Many-to-Many relationships)
    console.log("🔗 Seeding blog post tags...");
    const postTagRelations: NewBlogPostTag[] = [
      // Next.js post tags
      {
        postId: insertedPosts[0].id,
        tagId: insertedTags.find((t) => t.slug === "nextjs")!.id,
      },
      {
        postId: insertedPosts[0].id,
        tagId: insertedTags.find((t) => t.slug === "react")!.id,
      },
      {
        postId: insertedPosts[0].id,
        tagId: insertedTags.find((t) => t.slug === "typescript")!.id,
      },

      // Web design post tags
      {
        postId: insertedPosts[1].id,
        tagId: insertedTags.find((t) => t.slug === "ui-design")!.id,
      },
      {
        postId: insertedPosts[1].id,
        tagId: insertedTags.find((t) => t.slug === "ux-design")!.id,
      },
      {
        postId: insertedPosts[1].id,
        tagId: insertedTags.find((t) => t.slug === "css")!.id,
      },

      // Startup post tags
      {
        postId: insertedPosts[2].id,
        tagId: insertedTags.find((t) => t.slug === "startup")!.id,
      },
      ...
      {
        postId: insertedPosts[3].id,
        tagId: insertedTags.find((t) => t.slug === "javascript")!.id,
      },
    ];

    await db.insert(blogPostsTags).values(postTagRelations);
